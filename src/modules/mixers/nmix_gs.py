...
class Mixer(nn.Module):
    def __init__(self, args, abs=True):
        ...
        self.hyper_b1 = nn.Sequential(nn.Linear(self.input_dim, self.embed_dim * self.n_agents))
        ...
        print("Inside nmix_gs")

    def forward(self, qvals, states):
        # reshape
        b, t, _, _ = qvals.size()
        
        qvals = qvals.reshape(b * t, self.n_agents, self.n_agents)
        states = states.reshape(-1, self.state_dim)

        # First layer
        w1 = self.hyper_w1(states).view(-1, self.n_agents, self.embed_dim) # b * t, n_agents, emb
        b1 = self.hyper_b1(states).view(-1, self.n_agents, self.embed_dim)
        
        # Second layer
        w2 = self.hyper_w2(states).view(-1, self.embed_dim, 1) # b * t, emb, 1
        b2= self.hyper_b2(states).view(-1, self.n_agents, 1)
        ...
        return y.view(b, t, 1, self.n_agents)
