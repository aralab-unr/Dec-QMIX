...
class Agents:
    def __init__ (self, n_agents, x_len = 100, y_len = 100, desired_comm_dist = 30, fire_origin = None, int_type1 = np.int8, int_type2 = np.int16):
    ...
    def choose_agent_init_pos(self, fire_origin):
        ...
        # Determine the opposite boundary based on the location of point A
        if x > self.x_len // 2:  # Left side of the grid
            possible_x = 5
        else:  # Right side of the grid
            possible_x = self.x_len - 6

        if y > self.y_len // 2:  # Top side of the grid
            possible_y = 5
        else:  # Bottom side of the grid
            possible_y = self.y_len - 6

        # Randomly choose whether to prioritize the x-boundary or y-boundary
        if np.random.rand() > 0.5:
            # Use the x boundary, and randomize the y-coordinate
            point_b = (possible_x, np.random.randint(0, self.y_len))
        else:
            # Use the y boundary, and randomize the x-coordinate
            point_b = (np.random.randint(0, self.x_len), possible_y)

        return point_b

    def reset_position(self, fire_origin):
        self.positions = np.array([[5, 55], [5, 60], [5, 65]]).astype(self.int_type2)
...
class WildfireEnvironment(gym.Env):
    ...
    def __init__ (
        ...
        self.n_agents = n_agents
        self.agents = Agents(self.n_agents, self.x_len, self.y_len, self.desired_comm_dist, self.fire_origin, self.int_type1, self.int_type2)
    ...
    def reset(self, test_case_id = 1):
        ...
        self.binary_val = np.load("binary_val.npy")
        self.fire_set = np.load("fire_set.npy")
        self.fire_off = np.load("fire_off.npy")
        self.universal_fire_set = np.load("universal_fire_set.npy")
        ...
        self.fire_trajectories = [{"fire_on" : self.fire_set, "fire_off" : self.fire_off}]
    ...
    def get_state(self): 
        return np.expand_dims(np.append(self.agents_states.flatten(), 
                [self.fire_origin[0], self.fire_origin[1], 
                 np.mean(np.linalg.norm(np.array([self.fire_origin] * len(self.fire_set)) - self.fire_set, axis = 1))
                ]),
            axis = 0
        )
    ...
    def simStep(self):
        ...
        for ni, nj in new_fire_coords:
            ...
            self.universal_fire_set.add((ni, nj))
    ...
    def step(self, actions):
        ...
        self.fire_trajectories.append({"fire_on" : self.fire_set, "fire_off" : self.fire_off})
        ...
        return np.sum(rew), terminated, self.get_stats()
    ...
    def render(self, ep_n, n_ts = 0, path = ""):
    ...
    def get_stats(self):
        stats = {
            "coverage_perc": ( np.sum(self.recorded_fire_map.clip(max=1)) / len(self.universal_fire_set) ) * 100,
    ...
    def get_obs_size(self):
        ...
        obs_size = 2 + ( ( self.n_agents - 1 ) * 4 ) + self.n_agents + 9 + 3
        # obs_size = 2 + 9 + 3
        return obs_size
    ...
    def get_state_size(self):
        ...
        # state_size = ( 2 + 9 + 3 )
        return state_size * self.n_agents + 3
